...
namespace HogUtils {
    ...
    Mat visualize(const Mat& color_origImg, vector<float>& descriptorValues, const Size & size )
    {
    ...
    Mat compute(const Mat& img, const HOGDescriptor& hog, bool do_flip, string output) {
        Mat gray;
        ...
        // Save a visualization of the HoG
        if (output == "") {
            imwrite(output, HogUtils::visualize(img.clone(), descriptors, hog.winSize));
        }
